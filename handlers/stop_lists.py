...
async def dish_added(user_id, actor, dish_id):
    ...
    db.set_stop_list( str(stop_list))
...
def stop_list_status(actor):
    ...
    if actor == "admin":
        ...
        btn3 = InlineKeyboardButton(text="⬅️ Назад", callback_data="back_to_admin_menu")
    else:
        btn2 = InlineKeyboardButton(text="-", callback_data="boss_d_from_stop_list")
        btn3 = InlineKeyboardButton(text="⬅️ Назад", callback_data="back_to_boss_menu")
...
def change_stop_list(stop_list, actor):
    ...
    for dish in stop_list:
        ...
        keyboard.row(btn)
    if actor == "admin":
        btn1 = InlineKeyboardButton(text="⬅️Назад", callback_data="admin_back_to_stop_list")
    else:
        btn1 = InlineKeyboardButton(text="⬅️ Назад", callback_data="boss_back_to_stop_list")
    keyboard.row(btn1)
    return keyboard
